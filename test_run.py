...
class DataLoader:
    ...
    def load_trajectory(
        ...
        df = duckdb.query(
            ...
            SELECT lon, lat, TO_TIMESTAMP(time) AS timestamp, 
                   edge_id, reversed, raw_lane_id, mapped_lane_id
            FROM '{path}'
    ...
    def prepare_data(df: pd.DataFrame) -> PreparedData:
        ...
        return PreparedData(gps_measurements, coordinates, edge_ids)
...
class BenchmarkOrchestrator:
    ...
    async def run_vehicle_experiments(
        ...
        # Load ground truth data (shared across all matchers)
        gt_df = self.data_loader.load_trajectory(
...
async def main():
    ...
    NOISE_PARQUET = (
        ROOT_PATH
        / "sumo/simulations/ohare-chicago-junctionless/output/fcd_noisy_2.parquet"
    )
